...
class Userbot(Client):
    ...
    async def start(self):
        LOGGER(__name__).info(f"بدء تشغيل العميل")
        if config.STRING1:
            ...
            assistants.append(1)
            try:
                await self.one.send_message(config.LOG_GROUP_ID, "- تم بدء تشغيل المساعد .")
            except:
            ...
            LOGGER(__name__).info(f"بدء تشغيل المساعد {self.one.name}")
        if config.STRING2:
            ...
            assistants.append(2)
            try:
                await self.two.send_message(config.LOG_GROUP_ID, "- تم بدء تشغيل المساعد .")
            except:
                LOGGER(__name__).error(
            ...
            LOGGER(__name__).info(f"بدء تشغيل المساعد {self.two.name}")
        if config.STRING3:
            ...
            assistants.append(3)
            try:
                await self.three.send_message(config.LOG_GROUP_ID, "- تم بدء تشغيل المساعد .")
            except:
                LOGGER(__name__).error(
            ...
            LOGGER(__name__).info(f"بدء تشغيل المساعد {self.three.name}")
        if config.STRING4:
            ...
            assistants.append(4)
            try:
                await self.four.send_message(config.LOG_GROUP_ID, "- تم بدء تشغيل المساعد .")
            except:
                LOGGER(__name__).error(
            ...
            LOGGER(__name__).info(f"بدء تشغيل المساعد {self.four.name}")
        if config.STRING5:
            ...
            assistants.append(5)
            try:
                await self.five.send_message(config.LOG_GROUP_ID, "- تم بدء تشغيل المساعد .")
            except:
                LOGGER(__name__).error(
            ...
            LOGGER(__name__).info(f"بدء تشغيل المساعد {self.five.name}")
